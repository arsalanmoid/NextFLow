...
var AUTH_KEY = process.env.TRANSLOADIT_KEY;
var AUTH_SECRET = process.env.TRANSLOADIT_SECRET;
var VIDEO_EXTS = /* @__PURE__ */ new Set([".mp4", ".mov", ".webm", ".m4v", ".avi", ".mkv"]);
function buildSteps(filename) {
...
async function uploadToTransloadit(buffer, filename) {
  const expires = new Date(Date.now() + 5 * 60 * 1e3).toISOString();
  const params = JSON.stringify({
    auth: { key: AUTH_KEY, expires },
    steps: buildSteps(filename)
  });
  const signature = crypto.createHmac("sha384", AUTH_SECRET).update(params).digest("hex");
  ...
  const res = await fetch("https://api2.transloadit.com/assemblies", {
    ...
    body: formBuffer.buffer.slice(formBuffer.byteOffset, formBuffer.byteOffset + formBuffer.byteLength)
  });
  if (!res.ok) {
...
async function pollAssembly(assemblyUrl, maxAttempts = 60) {
...
__name(pollAssembly, "pollAssembly");
function extractUrl(assembly) {
  const steps = assembly.results ?? {};
  for (const stepResults of Object.values(steps)) {
    if (stepResults?.[0]) {
      return {
...
mime-types/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-VAEICCE6.mjs.map
